...
def extract_info_properties(subject_name: str) -> Tuple[str, str]:
    ...
    if not os.path.exists(info_properties_path):
        # try default names
        source_file_path = os.path.join(__location__, subject_name, "small.c")
        script_file_path = os.path.join(__location__, subject_name, "r.sh")
        if os.path.exists(source_file_path) and os.path.exists(script_file_path):
