...
def get_byte_size(file: str) -> int:
    benchmark_folder = os.path.dirname(file)
    source_file = [ f for f in os.listdir(benchmark_folder) if f.endswith('.c') or f.endswith('.rs') or f.endswith('smt2') or f.endswith('sy')]
    assert len(source_file) == 1
    source_file = os.path.join(benchmark_folder, source_file[0])
    with open(source_file, 'r') as f:
        content = f.read()
    return sum(len(x) for x in content.split())
...
def main():
    ...
    for folder in folders:
        ...
        # file_list.extend((benchmark_results_location))
        json_file = sorted(get_file_list(benchmark_results_location))
        if (json_file):
            file_list.append(sorted(get_file_list(benchmark_results_location))[-1])
